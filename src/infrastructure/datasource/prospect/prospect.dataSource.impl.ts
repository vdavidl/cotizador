import { ProspectDataSource, CustomError, ServerResponseEntity, EditProspectDto } from "../../../domain";
import { prisma } from "../../../data";
import { ProspectEntityMapper, ProspectListEntityMapper } from "../../mappers/prospect.mapper";
import { CreateProspectDto, IdDto } from "../../../domain";
import { QuotationEntityMapper, QuotationOfProspectEntityMapper } from "../../mappers";


export class ProspectDataSourceImpl implements ProspectDataSource{


    async showAllProspects(): Promise<ServerResponseEntity['props']> {//posibilidad de añadir filtros a la búsqueda
        try {
            ...
            const arrayProspectToEntity = prospectArray.map(ProspectListEntityMapper.prospectsToJson)    
    ...
    async showProspectDetail(idDto: IdDto) : Promise <ServerResponseEntity['props']>{
        try {
            const {id} = idDto; 
    ...
    async createProspect(createProspectDto : CreateProspectDto) : Promise <ServerResponseEntity['props']>{
        try {
            ...
            const { user_id, organization_name, contact_person, type, email, identity_document, whatsapp, first_contact_date, last_contact_date } = createProspectDto;
            const existProspect  = await this.getProspectByDni(identity_document);
            if(existProspect) throw CustomError.badRequest('Prospect dni already exist');

            const prospect = await prisma.prospect.create({
                data: {user_id_fk: user_id, organization_name, contact_person, type, email, identity_document, whatsapp, first_contact_date, last_contact_date}
            })
            return ServerResponseEntity.fromObject({
                message: 'Prospect created successfully', status: 'success', data: null, error: null
            });
        } catch (error) {
            console.log(`${error}`)
            throw CustomError.internalServer("Internal server error")
    ...
    async showProspectQuotation(prospectId: IdDto): Promise<ServerResponseEntity['props']>{
        try {
            ...
            if(!existprospectQuotations) throw CustomError.notFound('Quotations not found');
            


            const arrayQuotationstoEntity= existprospectQuotations.map(QuotationOfProspectEntityMapper.quotationToJson);
    ...
    private async getProspectByDni(dni : string){
        try {
            const prospect = await prisma.prospect.findFirst({where: { identity_document: dni }});
            return prospect;
        } catch (error) {
            console.log(`${error}`)
            throw CustomError.internalServer("Internal server error")
    ...
    private async getQuotationsByProspectId(id: number){
        try {
            const prospectQuotations = await prisma.quotation.findMany({where: {prospect_id_fk : id},
                select:{
                    id: true,
                    // service_id_fk: true,
                    net_amount: true,
