...
export class CompanyDataSourceImpl implements CompanyDataSource {
  async showCompanyData(): Promise<ServerResponseEntity["props"]> {
    try {
      ...
      if (!dataCompany || dataCompany.length === 0)
        throw CustomError.notFound("Company data doesnt exist");

      const dataCompanyToEntity = dataCompany.map(
        CompanyDataEntityMapper.companyDataToJson
      );
  ...
  async createCompanyData(
    createCompanyDataDto: CompanyDataDto
    ...
    try {
      ...
      if (existCompanyData && existCompanyData.length > 0)
        throw CustomError.badRequest("Data company already exist");
  ...
  async editCompanyData(
    ...
    try {
      ...
      if (!existCompanyData)
        throw CustomError.badRequest("Company data doesnt exist");
